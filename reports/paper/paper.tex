...
In addition to adding columns, the original dataset was filtered. Only trips
whose start and locations fell inside a rectangle that bounds Manhattan were
considered. As a preliminary filtering, trips with duration less than 0 or
greater than one day were removed. Upon training models, this was further
culled to trips between two minutes and two hours in length.

Principal component analysis was run on all de-meaned and
standardized geographic coordinates (both start
and end locations) from the filtered data. The coordinates were thus
transformed into the first and second principal components. For the discrete
grid models, these coordinates were further discretized into a $20 \times 70$
grid.

The data-set was saved as a whole, but also partioned into $(\text{day of week},
\text{hour})$ groups, using the one-hot encoded vectors. Some models were
trained on the entire data-set (e.g.\ Neural Network), and some were trained
individually on the partioned data-sets (e.g.\ Structural Models).

\subsection{Baselines}

Three baselines were calculated. The first was a linear regression predicting
...
layer with 50 neurons and a tanh activation function.

The second baseline was constructed by querying the Google Maps Distance Matrix
...
performance on 2,500 data points in a representative partioned test data-set.
...
The softmax structural model was trained
using the \texttt{Adam} optimizer with learning rate $10^{-2}$ for 10 epochs,
taking approximately an hour.
% TODO: jchen confirm this for me:
