...
\emph{before} the pick-up time, making for a trip with negative duration. Furthermore,
...
a taxi trip, we can only observe the start- and end-points. Therefore, our main inference challenge is the latent route choice of each trip.
...
Travel time prediction has historically been a topic of research interest. On a problem of freeway travel time prediction, \citet*{van2002freeway} proposed a Recurrent Neural Network (RNN) approach in order to address the temporal aspect of traffic prediction; \citet*{wu2004travel} adopted the Support Vector Regression (SVR) method applied to time-series analysis and reduced prediction errors in cases where previous methods generate especially large errors. We notice that earlier papers used freeway data for training and prediction, which probably arises from the limited availability of GPS data sources. Nevertheless, we realize that even for freeways, which in general have less congestion than city roads, there is evidence of non-linearity in the data, which contributes to the good performance of models such as RNN and SVR.

With the ubiquity of GPS data, recent work focuses on estimating the traffic conditions in cities using sparse probe data---a time series of closely spaced GPS location data for each trip---and uses travel time predictions as the metric as the performance of the model. \citet*{hunter2009path} used a Bayesian framework and used an Expectation-Maximization algorithm that simultaneously learns the likely paths taken by probe vehicles as well as the travel time distributions through the network; \citet*{herring2010estimating} used a Coupled Hidden Markov Model (CHMM) and determined the most likely path by allocating the travel time between two consecutive location observations to roads in the M-step of the EM algorithm.

Compared to previous work in the literature, we have much more missing information. Instead of sparse probe data, we are only working with start and end locations, which presents challenges in actual path inference and parameter optimization. \citet*{zhan2013urban} provide some helpful insights since they used the same TLC dataset of taxi ride. They constructed a faithful representation of the Manhattan streets network, but limited the discussion to a much smaller district in Manhattan to reduce computational complexity. Moreover, \citet*{zhan2013urban} also used a softmax regression model for path selection (as detailed in Section~\ref{sec:softmax}), but only restricting the choice set to the 20 shortest paths. Our contribution relative to \citet*{zhan2013urban} is twofold: First, our work scales to the entirety of Manhattan by sacrificing the minute realism of their model and discretizing the Manhattan road network as a simply a grid; second, we employ a novel sampling-based approach to estimate the softmax regression model, so as to drastically expand the choice set of drivers in our model.
...
\]
in order to encode the fact that drivers avoid routes that take a long period of time.\footnote{In general, we can have a \emph{temperature} parameter to the softmax that is not $1$. However, this is a hyperparameter since route choice is latent and thus not observable. To simplify notation and to be consistent with the applied parts of our project, we use softmax with temperature $1$.} In the \emph{uniform} model, we simply assume that route choice is independent and uniform on the set of reasonable paths: \[
...
\end{align*}
The expectation is a sum of the size $|\bm Z\uppr{k}|,$ which, for an $m\times n$ trip\footnote{By an $m\times n$ trip, we mean a trip with east-west distance $n$ and north-south distance $m$}, is of $\binom{m+n}{n} \approx \frac{(n+m)^n}{n^n}e^n$ terms.\footnote{Sterling's approximation} Computing this expectation is the main inference challenge of our project.
...
\end{equation}
where $\tilde Z$ is drawn uniformly from $\bm Z$. This is the same technique as in importance sampling \cite{gelman2014bayesian}, except here the objective is to derive computationally tractable approximations, rather than maximizing the efficiency of the approximations. We note that the same trick allows us to replace any sum over $\bm Z$ with a weighted sum over the uniformly chosen sample. Exchanging $\log$ and expectation operator and applying \eqref{eq:trick} yields a lower bound for log-likelihood, up to a constant \[
-\frac{1}{L}\bk{\sum_{\tilde Z} \frac{1}{2\sigma^2} \pr{T - \sum_{i\in \tilde Z} w_i}^2 + \log \frac{\exp\pr{-\sum_i w_i}}{\sum_{\bm Z}\exp\pr{-\sum_i w_i}}}.
\]
Using $\tilde Z$ again to approximate the sum in the denominator yields the following upper bound for negative log-likelihood, up to a constant,
\begin{align}
...
\label{sub:predict}
We conclude this section by briefly discussing prediction in our models. In the uniform model, prediction is straightforward. We calculate the expected time $\hat T = \sum_i w_i \pi_i$, where $\pi_i$ is the marginal probability that node $i$ is included in a uniformly randomly chosen path. Computing prediction is slightly more subtle in the softmax regression model. In the softmax regression model, we sample $L_{\text{pred}}$\footnote{In the implementation and application, we used $L_\text{pred} = 50$.} uniformly chosen paths and take the weighted average of the sums of $w_i$ in each path, where the weights are proportional to $\exp(-\sum_{i \in Z} w_i)$ for path $Z$. 
...
In addition to adding columns, we filtered the original dataset. We only
considered trips
whose start- and end-locations fall inside a rectangle that bounds Manhattan. We also removed trips with duration less than two minutes or more than two hours.

We performed a principal component analysis on all de-meaned and
standardized geographic coordinates (both start-
and end-locations) from the filtered data. We transformed the coordinates into
the first and second principal components. This allowed us to compute the
rotational matrix so that the major axis of Manhattan lies approximately orthogonal to the $x$-axis. For the discrete
...
individually on the partioned datasets (e.g.\ our structural models).

\subsection{Baselines}

We calculated three baselines. The first was a linear regression predicting
trip duration from Euclidian distance, and trained on a partitioned data set. 
...
constraint on how many test data points it could be tested on. We assessed its
performance on approximately 2,500 data points in a representative partioned
test dataset. Moreover, Google Maps API does not allow predicting travel time in
the past, and thus for each trip, we used a random Tuesday 8:00 AM in January 2018 as a proxy. From our tests, the predictions of Google Maps API are not significantly different, whether the user submits a start time or not. Thus, we believe that this is a reasonable approximation given limitations to the data.

\subsection{Training}

Training implementations varied from model to model. In training the neural
network baseline, we used SGD with a learning rate of $10^{-7}$ run for twelve 
...
hour. Convergence appeared after roughly two epochs, and the model was run for
five epochs taking just under ten minutes.
...
taking approximately thirty minutes for each data-subset. Additionally, the
softmax regression model takes hyperparameters $L$ (sample size in each
stochastic draw) and $\sigma^2$ (variance of trip time). We chose $L = 20$ and
$\sigma^2 = 100$. These were again chosen to strike a bargain between realism
and computation feasibility. The model appeared to converge by the second epoch. 
...
We train our model and test the model on an out-of-sample test-set using the
methods discussed in Section~\ref{sub:predict}. As a simple check of the
training process, we plot in Figure~\ref{fig:softmaxloss} the loss obtained when
fitting the softmax regression model. We see that the model converges relatively
fast.\footnote{The full fitting takes approximately 30 minutes per dataset (2.9
GHz Intel Core i5, 8GB RAM), but the model appears to converge in only one-fifth
of this time.} The convergence plot for the uniform model is extremely similar and thus omitted. Table~\ref{tab:mainres} contains the main prediction results of our model relative to some benchmark predictions
such as the Google Maps Distance Matrix API, neural network, and linear regression. We note that, in terms of predictive metrics, our models were able to outperform Google Maps and linear regression, but were not able to outperform the neural network, even though the difference is not very large. 
...
Table~\ref{tab:mainres} is encouraging. Our models successfully leverage the
power of geography and pick up more signal in the data than distance of the trip
alone, as demonstrated by the significantly improved performance over linear
regression. Perhaps more strikingly, our model slightly outperforms the
predictions generated by Google Maps, which means that the predictive accuracy
of the model is on par with the standard of a widely used consumer product. Even
though we were not able to outperform the neural network, we reiterate that the
main advantage of our approach is to have interpretable parameters, which are
visualized in Figure~\ref{fig:unifmap}.\footnote{In Figure~\ref{fig:unifmap}, the southern border of Central Park lies
approximately three-sevenths of the way up} We turn to the figure now.
% \begin{itemize}
...
superimposition of paths that we train on, but the result largely remains a
surprise.
...
this is a feature or a bug. It is also interesting to note that the Google Maps
predictions exhibit a bias in the opposite direction.
...
We now discuss a few directions for future work. Foremost among them is
taking a closer look at which of the softmax and
...
Finally, we note that our models condition only on hour and day of the week when
making predictions. However, our framework can incorporate a wider range of
relevant information. For instance, if weather data were accessible, the effect
of different weather conditions on road congestion could be measured by adding the weather covariates to the Normal mean. Similarly,
abnormal traffic conditions on holidays could be identified.
