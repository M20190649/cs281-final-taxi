...
\midrule
$\sd(T)$ & 7.40 & 6.85& &  6.22 & 6.22 \\ [1em]
$\hat \E[\epsilon]$ & -0.83 &-0.07 & &  0.61 & 0.99 \\
$\sd(\epsilon)$ & 4.98 & 3.88& &  4.28 & 4.28 \\[1em]
$\hat \E[|\epsilon|]$ & 3.42 & 2.44& &  2.82 & 2.86 \\
$\text{Median}(|\epsilon|)$ & 2.55 & 1.67& &  1.92 & 1.92 \\
$\text{Percentile}(|\epsilon|, 99)$ & 16.54 &12.79 & &  14.72 & 15.24 \\ [1em]
Test-set $R^2$ & 0.54 & 0.68&  & 0.53 & 0.53 \\
\bottomrule
\end{tabular}
\caption{Prediction results on test set}
\label{tab:mainres}
\begin{tablenotes}
\item \textbf{Notes:} Here $\epsilon = (\text{Actual trip duration}) - (\text{Predicted duration})$ in minutes. $\hat \E$ denotes the sample mean. We restrict the test-set to 8:00--9:00 AM on Tuesdays in January 2009 for all models \emph{except} the neural network, for which we use an out-of-sample test set on the \emph{entire} range of trip times, since the neural network encodes time-of-day and day-of-week data. Test-set $R^2$ is calculated as \smash{$1-\frac{\var(\epsilon)}{\var(T)}$} on the underlying dataset being tested on. Note that for all but the linear regression, the test-set $R^2$ is an out-of-sample $R^2$, whereas the linear regression is trained on the test set, and thus the $R^2$ reported is in-sample.
\end{tablenotes}
\end{threeparttable}
\end{table*}
